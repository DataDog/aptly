...
func (p *PublishedRepo) Publish(packagePool aptly.PackagePool, publishedStorageProvider aptly.PublishedStorageProvider,
	...
	for component, list := range lists {
		...
		for _, arch := range p.Architectures {
			for _, udeb := range udebs {
				release := make(Stanza)
				release.Set("Archive", p.Distribution)
				release.Set("Architecture", arch)
				release.Set("Component", component)
				release.Set("Origin", p.GetOrigin())
				release.Set("Label", p.GetLabel())
				release.Set("Suite", p.GetSuite())
				if p.AcquireByHash {
	...
	release.Set("Origin", p.GetOrigin())
	if p.NotAutomatic != "" {
		release.Set("NotAutomatic", p.NotAutomatic)
	}
	if p.ButAutomaticUpgrades != "" {
		release.Set("ButAutomaticUpgrades", p.ButAutomaticUpgrades)
	}
	release.Set("Label", p.GetLabel())
	release.Set("Suite", p.GetSuite())
	release.Set("Codename", p.Distribution)
	release.Set("Date", time.Now().UTC().Format("Mon, 2 Jan 2006 15:04:05 MST"))
	release.Set("Architectures", strings.Join(utils.StrSlicesSubstract(p.Architectures, []string{ArchitectureSource}), " "))
	if p.AcquireByHash {
		release.Set("Acquire-By-Hash", "yes")
	}
	release.Set("Description", " Generated by aptly\n")
	release.Set("MD5Sum", "")
	release.Set("SHA1", "")
	release.Set("SHA256", "")
	release.Set("SHA512", "")

	release.Set("Components", strings.Join(p.Components(), " "))
	...
	for _, path := range sortedPaths {
		...
		release["MD5Sum"].WriteString(fmt.Sprintf(" %s %8d %s\n", info.MD5, info.Size, path))
		release["SHA1"].WriteString(fmt.Sprintf(" %s %8d %s\n", info.SHA1, info.Size, path))
		release["SHA256"].WriteString(fmt.Sprintf(" %s %8d %s\n", info.SHA256, info.Size, path))
		release["SHA512"].WriteString(fmt.Sprintf(" %s %8d %s\n", info.SHA512, info.Size, path))
	}
